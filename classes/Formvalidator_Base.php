...
{    
    ...
    {
        $this->_rules[] = new ValidationRule_Base($fieldname, $message, $ruletype, $criteria);
    }
    */
    public function addRule($params)
    {
        if(is_array($params)){
            ...
            $this->_rules[] = new ValidationRule_Base($params);
        }
    ...
    public function addEntries($fields) {
        foreach ($fields as $fieldname => $value) {
            if(is_array($value)){
                foreach($value as $nameForm=>$valueForm){
                    $this->_fields[$nameForm] = $this->sanitize($valueForm);
                }
            }
        }
    }
    ...
    private function _testRule($rule) 
    {
       //echo '<pre>';
         ...
         * Supported rule types:    required, -- обовязковий параметр
         *                          minlength, -- мінімальна довжина
         *                          maxlength, -- максимальна довжина
         *                          numeric, -- номер
         *                          numeric-range, між числами [5,40]
         *                          in-array,
         *                          regex, --регулярний вираз
         *                          email,
         *                          phone, (099) - 341 - 4250/phonenumber
         *                          zip,
         *                          creditcard,
         *                          callback  — Проверяет, может ли значение переменной быть вызвано в качестве функции
         * 
         */
